...
def lake(land, x_up_left, y_up_left):  # au sommet d'une montagne au au fond d'une cuvette
    size_lake = random.randint(2, define.TAILLEAREA - 1)
    ...
    for i in range(scroll_x_start, scroll_x_end):  # a modifier selon l'orientation
        for j in range(scroll_y_start, scroll_y_end):
            key = rules.concat_int(x_up_left+coordinate_x+i, y_up_left+coordinate_y+j)
            land[key].etat = "River"
            land[key].topology = lake_altitude
    slop = random.choice(["up", "down"])
    if slop == "up":
        up_slop_for_lake(land, lake_altitude, land[rules.concat_int(x_up_left, y_up_left)])
    elif slop == "down":
        down_slop_for_lake(land, lake_altitude, land[rules.concat_int(x_up_left, y_up_left)])
